...
You can download a zip file of the data with the following terminal commands.
```bash
curl -L "https://drive.usercontent.google.com/download?id={1vW-GavIOd3qc49cGkgRBUXRuddtNWUgC}&confirm=xxx" -o chips.zip
unzip chips.zip
```
The `chips` folder is structured like the following:
```
chips/
├── train/
│   ├── LABEL
...
Note that within the `train` and `val` directories, each data type (`LABEL`, `S1RTC`, and `S2L2A`) has its own subdirectory. This folder also includes  `create_chips.py`, which is how the chips create chips from the attached GeoTIFs. 
