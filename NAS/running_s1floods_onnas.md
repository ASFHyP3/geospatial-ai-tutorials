...
**Make sure you follow the steps in [initial setup](./initial_setup.md) and [TerraMind env creation](./terramind_env_creation.md) before running this tutorial!**
...
## Monitor your job
You can use the `qstat` command and your job ID to query the status of your job (`qstat XXXXXX`). This will produce an output like:
```
TODO
```
...
TODO
```
As this message states, use the `qstat -fx XXXXXX` instead to query the final results of your job. If this command reports `0` for the `exit_code` field, congrats - your job completed successfully!
